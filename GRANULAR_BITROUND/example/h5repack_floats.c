 ...
 * filters applied. Used as input for the h5repack Granular BitRound test so
 * the filter's set_local() callback retains the filter in the pipeline (the
...
main(void)
{
     ...
     * non-trivial mantissa bits, so quantization by Granular BitRound is
     * visible across the entire dataset rather than only on values that
     * exceed the retained mantissa width. */
    for (i = 0; i < DIM0; i++)
        for (j = 0; j < DIM1; j++)
