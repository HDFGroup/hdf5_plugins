...
#define CCR_FLT_PRM_NBR 5             /**< Number of parameters sent to filter (in cd_params array). NB: keep identical with ccr.h:GRANULARBR_FLT_PRM_NBR */
...
static size_t H5Z_filter_granularbr(unsigned int flags, size_t cd_nelmts, const unsigned int cd_values[], size_t nbytes,
...
const H5Z_class2_t H5Z_GRANULARBR[1] = {{
    ...
    (H5Z_can_apply_func_t)(ccr_can_apply_granularbr), /**< Callback to determine if current variable meets filter criteria */
    (H5Z_set_local_func_t)(ccr_set_local_granularbr), /**< Callback to determine and set per-variable filter parameters */
    (H5Z_func_t)(H5Z_filter_granularbr),  /**< Function to implement filter */
}};

void ccr_gbr (const int nsd, const int type, const size_t sz, const int has_mss_val, ptr_unn mss_val, ptr_unn op1);
 ...
 */
size_t H5Z_filter_granularbr(unsigned int flags, size_t cd_nelmts,
 ...
 */
void ccr_gbr (const int nsd, const int type, const size_t sz, const int has_mss_val,
        ...
        case NC_FLOAT:
            ...
            for (idx = 0L; idx < sz; idx++) {
                if ((val = op1.fp[idx]) != mss_val_cmp_flt && u32_ptr[idx] != 0U) {
                    ...
                    prc_bnr_xpl_rqr =
                            mnt_fabs == 0.0 ? 0 : abs((int) floor(xpn_bs2 - bit_per_dgt * mnt_log10_fabs) - qnt_pwr); /* Protect against mnt = -0.0 */
