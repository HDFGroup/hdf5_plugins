...
macro (EXTERNAL_ZFP_LIBRARY compress_type libtype)
  ...
  H5ZFP_IMPORT_SET_LIB_OPTIONS (zfp "zfp" ${libtype} "")
...
macro (H5ZFP_SET_LIB_OPTIONS libtarget defaultlibname libtype)
  set (libname "${defaultlibname}")
  H5ZFP_SET_BASE_OPTIONS (${libtarget} ${libname} ${libtype})

  if (${libtype} MATCHES "SHARED")
    if (WIN32)
      set (LIBH5ZFP_VERSION ${H5ZFP_PACKAGE_VERSION_MAJOR})
    else ()
      set (LIBH5ZFP_VERSION ${H5ZFP_PACKAGE_VERSION})
    endif ()
    set_target_properties (${libtarget} PROPERTIES VERSION ${LIBH5ZFP_VERSION})
    if (WIN32)
        set (${libname} "${libname}-${H5ZFP_PACKAGE_SOVERSION}")
    else ()
        set_target_properties (${libtarget} PROPERTIES SOVERSION ${H5ZFP_PACKAGE_SOVERSION})
    endif ()
  endif ()

  #-- Apple Specific install_name for libraries
  if (APPLE)
    option (H5ZFP_BUILD_WITH_INSTALL_NAME "Build with library install_name set to the installation path" OFF)
    if (H5ZFP_BUILD_WITH_INSTALL_NAME)
      set_target_properties(${libtarget} PROPERTIES
          LINK_FLAGS "-current_version ${H5ZFP_PACKAGE_VERSION} -compatibility_version ${H5ZFP_PACKAGE_VERSION}"
          INSTALL_NAME_DIR "${CMAKE_INSTALL_PREFIX}/lib"
          BUILD_WITH_INSTALL_RPATH ${H5ZFP_BUILD_WITH_INSTALL_NAME}
      )
    endif ()
  endif ()
endmacro ()
...
#-------------------------------------------------------------------------------
macro (H5ZFP_SET_BASE_OPTIONS libtarget libname libtype)
  if (${libtype} MATCHES "SHARED")
    if (WIN32)
...
#-------------------------------------------------------------------------------
macro (H5ZFP_IMPORT_SET_LIB_OPTIONS libtarget libname libtype libversion)
  H5ZFP_SET_BASE_OPTIONS (${libtarget} ${libname} ${libtype})
...
macro (H5ZFP_README_PROPERTIES)
  ...
  configure_file (
      ${H5ZFP_RESOURCES_DIR}/README.txt.cmake.in
