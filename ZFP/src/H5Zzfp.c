...
#define ZFP_VERSION_NO                                                                                       \
    ZFP_VERSION_NO_(ZFP_VERSION_MAJOR, ZFP_VERSION_MINOR, ZFP_VERSION_PATCH, ZFP_VERSION_TWEAK)
...
#define H5Z_FILTER_ZFP_VERSION_STR                                                                           \
    H5Z_FILTER_ZFP_VERSION_STR_(H5Z_FILTER_ZFP_VERSION_MAJOR, H5Z_FILTER_ZFP_VERSION_MINOR,                  \
...
#define H5Z_FILTER_ZFP_VERSION_NO                                                                            \
    H5Z_FILTER_ZFP_VERSION_NO_(H5Z_FILTER_ZFP_VERSION_MAJOR, H5Z_FILTER_ZFP_VERSION_MINOR,                   \
                               H5Z_FILTER_ZFP_VERSION_PATCH)

#define H5Z_ZFP_PUSH_AND_GOTO(MAJ, MIN, RET, MSG)                                                            \
    do {                                                                                                     \
        H5Epush(H5E_DEFAULT, __FILE__, _funcname_, __LINE__, H5E_ERR_CLS, MAJ, MIN, MSG);                    \
        retval = RET;                                                                                        \
        goto done;                                                                                           \
    } while (0)
...
const H5Z_class2_t H5Z_ZFP[1] = {{
    ...
    "-" H5Z_FILTER_ZFP_VERSION_STR " (ZFP-" ZFP_VERSION_STRING ") "
...
H5Z_zfp_can_apply(hid_t dcpl_id, hid_t type_id, hid_t chunk_space_id)
{
    ...
    if (!(dsize == 4 || dsize == 8))
        H5Z_ZFP_PUSH_AND_GOTO(H5E_PLINE, H5E_BADTYPE, 0, "requires datatype size of 4 or 8");
    ...
    if (ndims_used == 0 || ndims_used > max_ndims)
        H5Z_ZFP_PUSH_AND_GOTO(H5E_PLINE, H5E_BADVALUE, 0, "chunk must have only 1...3 non-unity dimensions");
#else
    if (ndims_used == 0 || ndims_used > max_ndims)
        H5Z_ZFP_PUSH_AND_GOTO(H5E_PLINE, H5E_BADVALUE, 0, "chunk must have only 1...4 non-unity dimensions");
#endif
...
H5Z_zfp_set_local(hid_t dcpl_id, hid_t type_id, hid_t chunk_space_id)
{
    ...
    if (dclass == H5T_FLOAT) {
        ...
        H5Z_ZFP_PUSH_AND_GOTO(H5E_PLINE, H5E_BADTYPE, 0, "datatype class must be H5T_FLOAT or H5T_INTEGER");
    }
    ...
    switch (ndims_used) {
        case 1:
            dummy_field = Z zfp_field_1d(0, zt, dims_used[0]);
            break;
        case 2:
            dummy_field = Z zfp_field_2d(0, zt, dims_used[1], dims_used[0]);
            break;
        case 3:
            dummy_field = Z zfp_field_3d(0, zt, dims_used[2], dims_used[1], dims_used[0]);
            break;
#if ZFP_VERSION_NO >= 0x0540
        case 4:
            dummy_field = Z zfp_field_4d(0, zt, dims_used[3], dims_used[2], dims_used[1], dims_used[0]);
            break;
#endif
        default:
#if ZFP_VERSION_NO < 0x0530
    ...
    if (mem_cd_nelmts == 0) {
        ...
        else /* just use ZFP library defaults */
        {
            ...
            H5Pset_zfp_expert_cdata(ZFP_MIN_BITS, ZFP_MAX_BITS, ZFP_MAX_PREC, ZFP_MIN_EXP, mem_cd_nelmts,
                                    mem_cd_values);
        }
    }
    ...
    if (have_zfp_controls) {
        ...
        switch (mem_cd_values[0]) {
            ...
            case H5Z_ZFP_MODE_ACCURACY:
                ...
                break;
            case H5Z_ZFP_MODE_EXPERT:
                Z zfp_stream_set_params(dummy_zstr, mem_cd_values[2], mem_cd_values[3], mem_cd_values[4],
                                        (int)mem_cd_values[5]);
                break;
#if ZFP_VERSION_NO >= 0x0550
            case H5Z_ZFP_MODE_REVERSIBLE:
    ...
    if (0 > H5Pmodify_filter(dcpl_id, H5Z_FILTER_ZFP, flags, hdr_cd_nelmts, hdr_cd_values))
        H5Z_ZFP_PUSH_AND_GOTO(H5E_PLINE, H5E_BADVALUE, 0, "failed to modify cd_values");

    /* cleanup the dummy ZFP stuff we used to generate the header */
    Z zfp_field_free(dummy_field);
    dummy_field = 0;
    Z zfp_stream_close(dummy_zstr);
    dummy_zstr = 0;
    B stream_close(dummy_bstr);
    dummy_bstr = 0;
...
static int
get_zfp_info_from_cd_values(size_t cd_nelmts, unsigned int const *cd_values, uint64 *zfp_mode,
                            uint64 *zfp_meta, H5T_order_t *swap)
{
...
static int
zfp_codec_version_mismatch(unsigned int h5zfpver_from_cd_val_data_in_file,
...
static size_t
H5Z_filter_zfp(unsigned int flags, size_t cd_nelmts, const unsigned int cd_values[], size_t nbytes,
    ...
    if (flags & H5Z_FLAG_REVERSE) /* decompression */
    {
        ...
        switch (Z zfp_field_type(zfld)) {
            ...
            case zfp_type_double:
                ...
                break;
            default:
                H5Z_ZFP_PUSH_AND_GOTO(H5E_PLINE, H5E_BADTYPE, 0, "invalid datatype");
        }
        bsize *= dsize;
