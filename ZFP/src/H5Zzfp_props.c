...
#define H5Z_ZFP_PUSH_AND_GOTO(MAJ, MIN, RET, MSG)                                                            \
    do {                                                                                                     \
        H5Epush(H5E_DEFAULT, __FILE__, _funcname_, __LINE__, H5E_ERR_CLS_g, MAJ, MIN, MSG);                  \
        retval = RET;                                                                                        \
        goto done;                                                                                           \
    } while (0)

static herr_t
H5Pset_zfp(hid_t plist, int mode, ...)
{
    ...
    for (i = 0; i < H5Pget_nfilters(plist); i++) {
        ...
        if (0 <= (fid = H5Pget_filter(plist, i, &flags, &cd_nelmts, cd_values, 0, 0, 0)) &&
            ...
            if (0 > H5Premove_filter(plist, H5Z_FILTER_ZFP))
                H5Z_ZFP_PUSH_AND_GOTO(H5E_PLINE, H5E_BADVALUE, -1,
                                      "Unable to remove old ZFP filter from pipeline.");
            break;
        }
    }
...
herr_t
H5Pset_zfp_rate(hid_t plist, double rate)
{
    return H5Pset_zfp(plist, H5Z_ZFP_MODE_RATE, rate);
}

herr_t
H5Pset_zfp_precision(hid_t plist, unsigned int prec)
{
    return H5Pset_zfp(plist, H5Z_ZFP_MODE_PRECISION, prec);
}

herr_t
H5Pset_zfp_accuracy(hid_t plist, double acc)
{
    return H5Pset_zfp(plist, H5Z_ZFP_MODE_ACCURACY, acc);
}

herr_t
H5Pset_zfp_expert(hid_t plist, unsigned int minbits, unsigned int maxbits, unsigned int maxprec, int minexp)
{
    return H5Pset_zfp(plist, H5Z_ZFP_MODE_EXPERT, minbits, maxbits, maxprec, minexp);
}

herr_t
H5Pset_zfp_reversible(hid_t plist)
{
