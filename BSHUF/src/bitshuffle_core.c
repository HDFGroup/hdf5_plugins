...
int64_t
bshuf_trans_byte_elem_remainder(const void *in, void *out, const size_t size, const size_t elem_size,
                                const size_t start)
{
    ...
    if (size > start) {
        ...
        for (ii = start; ii + 7 < size; ii += 8) {
            for (jj = 0; jj < elem_size; jj++) {
                for (kk = 0; kk < 8; kk++) {
                    out_b[jj * size + ii + kk] = in_b[ii * elem_size + kk * elem_size + jj];
                }
            }
        }
...
int64_t
bshuf_trans_byte_elem_scal(const void *in, void *out, const size_t size, const size_t elem_size)
{
...
int64_t
bshuf_trans_bit_byte_remainder(const void *in, void *out, const size_t size, const size_t elem_size,
...
/* Transpose bits within bytes. */
int64_t
bshuf_trans_bit_byte_scal(const void *in, void *out, const size_t size, const size_t elem_size)
{
...
int64_t
bshuf_trans_elem(const void *in, void *out, const size_t lda, const size_t ldb, const size_t elem_size)
{
    ...
    for (ii = 0; ii < lda; ii++) {
        for (jj = 0; jj < ldb; jj++) {
            memcpy(&out_b[(jj * lda + ii) * elem_size], &in_b[(ii * ldb + jj) * elem_size], elem_size);
        }
    }
...
int64_t
bshuf_trans_bitrow_eight(const void *in, void *out, const size_t size, const size_t elem_size)
{
...
int64_t
bshuf_trans_bit_elem_scal(const void *in, void *out, const size_t size, const size_t elem_size)
{
 ...
 * the bytes. */
int64_t
bshuf_trans_byte_bitrow_scal(const void *in, void *out, const size_t size, const size_t elem_size)
{
    ...
    for (jj = 0; jj < elem_size; jj++) {
        for (ii = 0; ii < nbyte_row; ii++) {
            for (kk = 0; kk < 8; kk++) {
                out_b[ii * 8 * elem_size + jj * 8 + kk] = in_b[(jj * 8 + kk) * nbyte_row + ii];
            }
        }
    }
    return size * elem_size;
...
int64_t
bshuf_shuffle_bit_eightelem_scal(const void *in, void *out, const size_t size, const size_t elem_size)
{
...
int64_t
bshuf_untrans_bit_elem_scal(const void *in, void *out, const size_t size, const size_t elem_size)
{
...
int64_t
bshuf_trans_byte_elem_NEON_16(const void *in, void *out, const size_t size)
{
...
int64_t
bshuf_trans_byte_elem_NEON_32(const void *in, void *out, const size_t size)
{
...
int64_t
bshuf_trans_byte_elem_NEON_64(const void *in, void *out, const size_t size)
{
...
int64_t
bshuf_trans_byte_elem_NEON(const void *in, void *out, const size_t size, const size_t elem_size)
{
    ...
    // Multiple of power of 2: transpose hierarchically.
    {
        ...
        if ((elem_size % 8) == 0) {
            ...
            count = bshuf_trans_byte_elem_NEON_64(out, tmp_buf, size * nchunk_elem);
            ...
            count = bshuf_trans_byte_elem_NEON_32(out, tmp_buf, size * nchunk_elem);
            ...
            count = bshuf_trans_byte_elem_NEON_16(out, tmp_buf, size * nchunk_elem);
 ...
 */
int32_t
move_byte_mask_neon(uint8x16_t input)
{

    return (((input[0] & 0x80) >> 7) | (((input[1] & 0x80) >> 7) << 1) | (((input[2] & 0x80) >> 7) << 2) |
            (((input[3] & 0x80) >> 7) << 3) | (((input[4] & 0x80) >> 7) << 4) |
            (((input[5] & 0x80) >> 7) << 5) | (((input[6] & 0x80) >> 7) << 6) |
            (((input[7] & 0x80) >> 7) << 7) | (((input[8] & 0x80) >> 7) << 8) |
            (((input[9] & 0x80) >> 7) << 9) | (((input[10] & 0x80) >> 7) << 10) |
            (((input[11] & 0x80) >> 7) << 11) | (((input[12] & 0x80) >> 7) << 12) |
            (((input[13] & 0x80) >> 7) << 13) | (((input[14] & 0x80) >> 7) << 14) |
            (((input[15] & 0x80) >> 7) << 15));
}

/* Transpose bits within bytes. */
int64_t
bshuf_trans_bit_byte_NEON(const void *in, void *out, const size_t size, const size_t elem_size)
{
    ...
    count = bshuf_trans_bit_byte_remainder(in, out, size, elem_size, nbyte - nbyte % 16);
    return count;
}

/* Transpose bits within elements. */
int64_t
bshuf_trans_bit_elem_NEON(const void *in, void *out, const size_t size, const size_t elem_size)
{
 ...
 * the bytes. */
int64_t
bshuf_trans_byte_bitrow_NEON(const void *in, void *out, const size_t size, const size_t elem_size)
{
...
/* Shuffle bits within the bytes of eight element blocks. */
int64_t
bshuf_shuffle_bit_eightelem_NEON(const void *in, void *out, const size_t size, const size_t elem_size)
{
...
/* Untranspose bits within elements. */
int64_t
bshuf_untrans_bit_elem_NEON(const void *in, void *out, const size_t size, const size_t elem_size)
{
...
/* Transpose bytes within elements for 16 bit elements. */
int64_t
bshuf_trans_byte_elem_SSE_16(const void *in, void *out, const size_t size)
{
...
/* Transpose bytes within elements for 32 bit elements. */
int64_t
bshuf_trans_byte_elem_SSE_32(const void *in, void *out, const size_t size)
{
...
/* Transpose bytes within elements for 64 bit elements. */
int64_t
bshuf_trans_byte_elem_SSE_64(const void *in, void *out, const size_t size)
{
...
int64_t
bshuf_trans_byte_elem_SSE(const void *in, void *out, const size_t size, const size_t elem_size)
{
...
/* Transpose bits within bytes. */
int64_t
bshuf_trans_bit_byte_SSE(const void *in, void *out, const size_t size, const size_t elem_size)
{
...
/* Transpose bits within elements. */
int64_t
bshuf_trans_bit_elem_SSE(const void *in, void *out, const size_t size, const size_t elem_size)
{
 ...
 * the bytes. */
int64_t
bshuf_trans_byte_bitrow_SSE(const void *in, void *out, const size_t size, const size_t elem_size)
{
...
/* Shuffle bits within the bytes of eight element blocks. */
int64_t
bshuf_shuffle_bit_eightelem_SSE(const void *in, void *out, const size_t size, const size_t elem_size)
{
...
/* Untranspose bits within elements. */
int64_t
bshuf_untrans_bit_elem_SSE(const void *in, void *out, const size_t size, const size_t elem_size)
{
...
/* Transpose bits within bytes. */
int64_t
bshuf_trans_bit_byte_AVX(const void *in, void *out, const size_t size, const size_t elem_size)
{
    ...
    count = bshuf_trans_bit_byte_remainder(in, out, size, elem_size, nbyte - nbyte % 32);
    return count;
}

/* Transpose bits within elements. */
int64_t
bshuf_trans_bit_elem_AVX(const void *in, void *out, const size_t size, const size_t elem_size)
{
 ...
 * the bytes. */
int64_t
bshuf_trans_byte_bitrow_AVX(const void *in, void *out, const size_t size, const size_t elem_size)
{
    ...
    for (jj = 0; jj + 31 < nbyte_row; jj += 32) {
        for (ii = 0; ii + 3 < elem_size; ii += 4) {
            for (hh = 0; hh < 4; hh++) {
                ...
                for (kk = 0; kk < 8; kk++) {
                    ymm_0[kk] = _mm256_loadu_si256((__m256i *)&in_b[(ii * 8 + hh * 8 + kk) * nbyte_row + jj]);
                }
                ...
                for (kk = 0; kk < 2; kk++) {
                    for (mm = 0; mm < 2; mm++) {
                        ymm_0[kk * 4 + mm] =
                            _mm256_unpacklo_epi16(ymm_1[kk * 4 + mm * 2], ymm_1[kk * 4 + mm * 2 + 1]);
                        ymm_0[kk * 4 + mm + 2] =
                            _mm256_unpackhi_epi16(ymm_1[kk * 4 + mm * 2], ymm_1[kk * 4 + mm * 2 + 1]);
                    }
                }
            ...
            for (mm = 0; mm < 8; mm++) {
                ...
                _mm256_storeu_si256((__m256i *)&out_b[(jj + mm * 2 + 0 * 16) * nrows + ii * 8], ymm_0[0]);
                _mm256_storeu_si256((__m256i *)&out_b[(jj + mm * 2 + 0 * 16 + 1) * nrows + ii * 8], ymm_0[1]);
                _mm256_storeu_si256((__m256i *)&out_b[(jj + mm * 2 + 1 * 16) * nrows + ii * 8], ymm_0[2]);
                _mm256_storeu_si256((__m256i *)&out_b[(jj + mm * 2 + 1 * 16 + 1) * nrows + ii * 8], ymm_0[3]);
            }
        }
    }
...
/* Shuffle bits within the bytes of eight element blocks. */
int64_t
bshuf_shuffle_bit_eightelem_AVX(const void *in, void *out, const size_t size, const size_t elem_size)
{
...
/* Untranspose bits within elements. */
int64_t
bshuf_untrans_bit_elem_AVX(const void *in, void *out, const size_t size, const size_t elem_size)
{
...
/* Transpose bits within bytes. */
int64_t
bshuf_trans_bit_byte_AVX512(const void *in, void *out, const size_t size, const size_t elem_size)
{
    ...
    count = bshuf_trans_bit_byte_remainder(in, out, size, elem_size, nbyte - nbyte % 64 % 32);
...
/* Transpose bits within elements. */
int64_t
bshuf_trans_bit_elem_AVX512(const void *in, void *out, const size_t size, const size_t elem_size)
{
...
/* Shuffle bits within the bytes of eight element blocks. */
int64_t
bshuf_shuffle_bit_eightelem_AVX512(const void *in, void *out, const size_t size, const size_t elem_size)
{
...
/* Untranspose bits within elements. */
int64_t
bshuf_untrans_bit_elem_AVX512(const void *in, void *out, const size_t size, const size_t elem_size)
{
...
int64_t
bshuf_trans_bit_elem(const void *in, void *out, const size_t size, const size_t elem_size)
{
...
int64_t
bshuf_untrans_bit_elem(const void *in, void *out, const size_t size, const size_t elem_size)
{
 ...
 * parallel. */
int64_t
bshuf_blocked_wrap_fun(bshufBlockFunDef fun, const void *in, void *out, const size_t size,
                       const size_t elem_size, size_t block_size, const int option)
{
...
#if defined(_OPENMP)
#pragma omp parallel for schedule(dynamic, 1) private(count) reduction(+ : cum_count)
#endif
...
int64_t
bshuf_bitshuffle_block(ioc_chain *C_ptr, const size_t size, const size_t elem_size, const int option)
{
...
int64_t
bshuf_bitunshuffle_block(ioc_chain *C_ptr, const size_t size, const size_t elem_size, const int option)
{
...
void
bshuf_write_uint64_BE(void *buf, uint64_t num)
{
    int      ii;
...
void
bshuf_write_uint32_BE(void *buf, uint32_t num)
{
    int      ii;
    uint8_t *b     = (uint8_t *)buf;
    uint32_t pow28 = 1 << 8;
...
uint32_t
bshuf_read_uint32_BE(const void *buf)
{
    int      ii;
    uint8_t *b   = (uint8_t *)buf;
...
size_t
bshuf_default_block_size(const size_t elem_size)
{
...
int64_t
bshuf_bitshuffle(const void *in, void *out, const size_t size, const size_t elem_size, size_t block_size)
{

    return bshuf_blocked_wrap_fun(&bshuf_bitshuffle_block, in, out, size, elem_size, block_size,
                                  0 /*option*/);
}

int64_t
bshuf_bitunshuffle(const void *in, void *out, const size_t size, const size_t elem_size, size_t block_size)
{

    return bshuf_blocked_wrap_fun(&bshuf_bitunshuffle_block, in, out, size, elem_size, block_size,
                                  0 /*option*/);
}
