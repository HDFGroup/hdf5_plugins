...
int64_t
bshuf_compress_lz4_block(ioc_chain *C_ptr, const size_t size, const size_t elem_size, const int option)
{
    ...
    nbytes = LZ4_compress_default((const char *)tmp_buf_bshuf, (char *)tmp_buf_lz4, size * elem_size,
                                  dst_capacity);
    free(tmp_buf_bshuf);
...
int64_t
bshuf_decompress_lz4_block(ioc_chain *C_ptr, const size_t size, const size_t elem_size, const int option)
{
...
/* Bitshuffle and compress a single block. */
int64_t
bshuf_compress_zstd_block(ioc_chain *C_ptr, const size_t size, const size_t elem_size, const int comp_lvl)
{
    ...
    nbytes = ZSTD_compress(tmp_buf_zstd, tmp_buf_zstd_size, (const void *)tmp_buf_bshuf, size * elem_size,
                           comp_lvl);
    free(tmp_buf_bshuf);
...
/* Decompress and bitunshuffle a single block. */
int64_t
bshuf_decompress_zstd_block(ioc_chain *C_ptr, const size_t size, const size_t elem_size, const int option)
{
...
int64_t
bshuf_compress_lz4(const void *in, void *out, const size_t size, const size_t elem_size, size_t block_size)
{
...
#ifdef ZSTD_SUPPORT
size_t
bshuf_compress_zstd_bound(const size_t size, const size_t elem_size, size_t block_size)
{
    ...
    leftover = ((size % block_size) / BSHUF_BLOCKED_MULT) * BSHUF_BLOCKED_MULT;
    if (leftover)
        bound += ZSTD_compressBound(leftover * elem_size) + 4;
    // Size of uncompressed data not fitting into any blocks.
    bound += (size % BSHUF_BLOCKED_MULT) * elem_size;
    return bound;
}
...
int64_t
bshuf_compress_zstd(const void *in, void *out, const size_t size, const size_t elem_size, size_t block_size,
                    const int comp_lvl)
{
    return bshuf_blocked_wrap_fun(&bshuf_compress_zstd_block, in, out, size, elem_size, block_size, comp_lvl);
}

int64_t
bshuf_decompress_zstd(const void *in, void *out, const size_t size, const size_t elem_size, size_t block_size)
{
    return bshuf_blocked_wrap_fun(&bshuf_decompress_zstd_block, in, out, size, elem_size, block_size,
                                  0 /*option*/);
}
#endif // ZSTD_SUPPORT
