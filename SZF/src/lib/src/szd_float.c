...
void decompressDataSeries_float_1D_MSST19(float** data, size_t dataSeriesLength, TightDataPointStorageF* tdps)
{
    ...
    *data = (float*)malloc(sizeof(float)*dataSeriesLength);
    ...
    int ii;
    for(ii=0; ii<exe_params->intvCapacity; ii++){
        double test = pow((1+tdps->realPrecision), inv*(ii - exe_params->intvRadius));
        precisionTable[ii] = test;
    }
...
void decompressDataSeries_float_2D_MSST19(float** data, size_t r1, size_t r2, TightDataPointStorageF* tdps)
{
    ...
    *data = (float*)malloc(sizeof(float)*dataSeriesLength);
    ...
    double inv = 2.0-pow(2, -(tdps->plus_bits));
    int iii;
    for(iii=0; iii<exe_params->intvCapacity; iii++){
        double test = pow((1+tdps->realPrecision), inv*(iii - exe_params->intvRadius));
...
void decompressDataSeries_float_2D_nonblocked_with_blocked_regression(float** data, size_t r1, size_t r2, unsigned char* comp_data, float* hist_data){
    ...
    *data = (float*)malloc(sizeof(float)*num_elements);
    ...
    if(reg_count > 0){
        int ii;
        for(ii=0; ii<3; ii++){
            precision[ii] = bytesToFloat(comp_data_pos);
            comp_data_pos += sizeof(float);
            coeff_intvRadius[ii] = bytesToInt_bigEndian(comp_data_pos);
            comp_data_pos += sizeof(int);
            unsigned int tree_size = bytesToInt_bigEndian(comp_data_pos);
            comp_data_pos += sizeof(int);
            int stateNum = 2*coeff_intvRadius[ii]*2;
            HuffmanTree* huffmanTree = createHuffmanTree(stateNum);
            int nodeCount = bytesToInt_bigEndian(comp_data_pos);
            node root = reconstruct_HuffTree_from_bytes_anyStates(huffmanTree, comp_data_pos+sizeof(int), nodeCount);
            comp_data_pos += sizeof(int) + tree_size;

            coeff_type[ii] = coeff_result_type + ii * num_blocks;
            size_t typeArray_size = bytesToSize(comp_data_pos);
            decode(comp_data_pos + sizeof(size_t), reg_count, root, coeff_type[ii]);
            comp_data_pos += sizeof(size_t) + typeArray_size;
            int coeff_unpred_count = bytesToInt_bigEndian(comp_data_pos);
            comp_data_pos += sizeof(int);
            coeff_unpred_data[ii] = (float *) comp_data_pos;
            ...
            for(j=0; j<num_y; j++){
                ...
                if(*indicator_pos){
                    ...
                    // decompress by regression
                    {
                        ...
                        int ee;
                        for(ee=0; ee<3; ee++){
                            type_ = coeff_type[ee][coeff_index];
                            if (type_ != 0){
                                pred = last_coefficients[ee];
                                last_coefficients[ee] = pred + 2 * (type_ - coeff_intvRadius[ee]) * precision[ee];
                            }
                            else{
                                last_coefficients[ee] = coeff_unpred_data[ee][coeff_unpred_data_count[ee]];
                                coeff_unpred_data_count[ee] ++;
                            }
                        }
                        coeff_index ++;
                    }
                    ...
                    if(*indicator_pos){
                        ...
                        // ii == 0
                        {
                            // jj == 0
                            {
                                ...
                                for(kk=1; kk<current_blockcount_z; kk++){
                                    type_ = type[index];
                                    if(type_ == intvRadius){
                                        *block_data_pos = mean;
                                    }
                            ...
                            for(jj=1; jj<current_blockcount_y; jj++){
                                {
                                    // kk == 0
                                    type_ = type[index];
                                    if(type_ == intvRadius){
                        ...
                        for(ii=1; ii<current_blockcount_x; ii++){
                            // jj == 0
                            {
                                {
                                    // kk == 0
...
void decompressDataSeries_float_3D_random_access_with_blocked_regression(float** data, size_t r1, size_t r2, size_t r3, unsigned char* comp_data){
    ...
    *data = (float*)malloc(sizeof(float)*num_elements);
    ...
    if(reg_count > 0){
        int iii;
        for(iii=0; iii<4; iii++){
            precision[iii] = bytesToDouble(comp_data_pos);
            comp_data_pos += sizeof(double);
            coeff_intvRadius[iii] = bytesToInt_bigEndian(comp_data_pos);
            comp_data_pos += sizeof(int);
            unsigned int tree_size = bytesToInt_bigEndian(comp_data_pos);
            comp_data_pos += sizeof(int);
            int stateNum = 2*coeff_intvRadius[iii]*2;
            HuffmanTree* huffmanTree = createHuffmanTree(stateNum);
            int nodeCount = bytesToInt_bigEndian(comp_data_pos);
            node root = reconstruct_HuffTree_from_bytes_anyStates(huffmanTree, comp_data_pos+sizeof(int), nodeCount);
            comp_data_pos += sizeof(int) + tree_size;
            ...
            size_t typeArray_size = bytesToSize(comp_data_pos);
            decode(comp_data_pos + sizeof(size_t), reg_count, root, coeff_type[iii]);
            comp_data_pos += sizeof(size_t) + typeArray_size;
            int coeff_unpred_count = bytesToInt_bigEndian(comp_data_pos);
            comp_data_pos += sizeof(int);
            coeff_unpred_data[iii] = (float *) comp_data_pos;
            comp_data_pos += coeff_unpred_count * sizeof(float);
            SZ_ReleaseHuffman(huffmanTree);
        }
    }
            ...
            for(j=0; j<num_y; j++){
                for(k=0; k<num_z; k++){
                    ...
                    for(iii=0; iii<block_size; iii++){
                        if(i*block_size + iii >= r1) break;
                        block_data_pos_y = block_data_pos_x;
                        for(jjj=0; jjj<block_size; jjj++){
                            if(j*block_size + jjj >= r2) break;
                            block_data_pos_z = block_data_pos_y;
                            for(kkk=0; kkk<block_size; kkk++){
                                if(k*block_size + kkk >= r3) break;
                                *block_data_pos_z = data_pos[iii*dec_buffer_size*dec_buffer_size + jjj*dec_buffer_size + kkk];
                                block_data_pos_z ++;
                            }
...
void decompressDataSeries_float_2D_decompression_given_areas_with_blocked_regression(float** data, size_t r1, size_t r2, size_t s1, size_t s2, size_t e1, size_t e2, unsigned char* comp_data){
        ...
        for(i=sx; i<ex; i++){
            for(j=sy; j<ey; j++){
                ...
                for(iii=0; iii<block_size; iii++){
                    ...
                    for(jjj=0; jjj<block_size; jjj++){
                        if(j*block_size + jjj >= r2) break;
                        *block_data_pos_y = data_pos[iii*dec_buffer_size + jjj];
                        block_data_pos_y ++;
                    }
    ...
    *data = (float*) malloc(sizeof(float)*(e1 - s1) * (e2 - s2));
    float * final_data_pos = *data;
    for(iii=0; iii<(e1 - s1); iii++){
        float * block_data_pos = dec_block_data + (iii+resi_x)*dec_block_dim0_offset + resi_y;
        for(jjj=0; jjj<(e2 - s2); jjj++){
            *(final_data_pos++) = *(block_data_pos++);
        }
    }
    free(dec_block_data);
...
void decompressDataSeries_float_3D_decompression_given_areas_with_blocked_regression(float** data, size_t r1, size_t r2, size_t r3, size_t s1, size_t s2, size_t s3, size_t e1, size_t e2, size_t e3, unsigned char* comp_data){
    ...
    *data = (float*) malloc(sizeof(float)*(e1 - s1) * (e2 - s2) * (e3 - s3));
    ...
    for(iii=0; iii<(e1 - s1); iii++){
        for(jjj=0; jjj<(e2 - s2); jjj++){
            float * block_data_pos = dec_block_data + (iii+resi_x)*dec_block_dim0_offset + (jjj+resi_y)*dec_block_dim1_offset + resi_z;
            for(kkk=0; kkk<(e3 - s3); kkk++){
                *(final_data_pos++) = *(block_data_pos++);
            }
        }
    }
