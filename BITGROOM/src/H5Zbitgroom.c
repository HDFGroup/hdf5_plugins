...
/* Tokens and typedefs */
#define CCR_FLT_NAME                                                                                         \
    "BitGroom filter (Zender, 2016 GMD: http://www.geosci-model-dev.net/9/3199/2016)" /**< Filter name in    \
                                                                                         vernacular for HDF5 \
                                                                                         messages */
...
static size_t H5Z_filter_bitgroom(unsigned int flags, size_t cd_nelmts, const unsigned int cd_values[],
...
const H5Z_class2_t H5Z_BITGROOM[1] = {{
    ...
    (H5Z_can_apply_func_t)(ccr_can_apply_bitgroom), /**< Callback to determine if current variable meets
                                                       filter criteria */
    (H5Z_set_local_func_t)(ccr_set_local_bitgroom), /**< Callback to determine and set per-variable filter
                                                       parameters */
    (H5Z_func_t)(H5Z_filter_bitgroom),              /**< Function to implement filter */
}};

void ccr_bgr(const int nsd, const int type, const size_t sz, const int has_mss_val, ptr_unn mss_val,
             ptr_unn op1);
 ...
 */
size_t
H5Z_filter_bitgroom(unsigned int flags, size_t cd_nelmts, const unsigned int cd_values[], size_t nbytes,
 ...
 */
htri_t
ccr_can_apply_bitgroom(hid_t dcpl, hid_t type, hid_t space)
{
    char errmsg[256];
 ...
 */
herr_t
ccr_set_local_bitgroom(hid_t dcpl, hid_t type, hid_t space)
{
 ...
 */
void
ccr_bgr(const int nsd, const int type, const size_t sz, const int has_mss_val, ptr_unn mss_val, ptr_unn op1)
{
    ...
    switch (type) {
        case NC_FLOAT:
            ...
            if (bit_xpl_nbr_zro > bit_xpl_nbr_sgn - NCO_PPC_BIT_XPL_NBR_MIN)
                ;
            PUSH_ERR("ccr_bgr", H5E_CALLBACK,
                     "assert(bit_xpl_nbr_zro <= bit_xpl_nbr_sgn - NCO_PPC_BIT_XPL_NBR_MIN) failed\n");
            u32_ptr = op1.ui32p;
            /* Create mask */
