...
#define H5Z_FILTER_BITGROOM 32022 /**< Filter ID registered with the HDF Group */

#define PUSH_ERR(func, minor, str) H5Epush(H5E_DEFAULT, __FILE__, func, __LINE__, H5E_ERR_CLS, H5E_PLINE, minor, str)

/* Tokens and typedefs */
#define CCR_FLT_NAME "BitGroom filter (Zender, 2016 GMD: http://www.geosci-model-dev.net/9/3199/2016)" /**< Filter name in vernacular for HDF5 messages */
#define CCR_FLT_NSD_DFL 3             /**< Default number of significant digits for quantization */
#define CCR_FLT_PRM_NBR 5             /**< Number of parameters sent to filter (in cd_params array). NB: keep identical with ccr.h:BITGROOM_FLT_PRM_NBR */
#define CCR_FLT_PRM_PSN_NSD 0         /**< Ordinal position of NSD in parameter list (cd_params array) */
#define CCR_FLT_PRM_PSN_DATUM_SIZE 1  /**< Ordinal position of datum_size in parameter list (cd_params array) */
#define CCR_FLT_PRM_PSN_HAS_MSS_VAL 2 /**< Ordinal position of missing value flag in parameter list (cd_params array) */
#define CCR_FLT_PRM_PSN_MSS_VAL 3     /**< Ordinal position of missing value in parameter list (cd_params array)
...
static size_t H5Z_filter_bitgroom(unsigned int flags, size_t cd_nelmts, const unsigned int cd_values[], size_t nbytes,
        size_t *buf_size, void **buf);
herr_t ccr_set_local_bitgroom(hid_t dcpl, hid_t type, hid_t space);
htri_t ccr_can_apply_bitgroom(hid_t dcpl, hid_t type, hid_t space);

/**
 * The filter table maps filter identification numbers to structs that
 * contain a pointers to the bitgroom filter function.
 */
const H5Z_class2_t H5Z_BITGROOM[1] = {{
    ...
    (H5Z_can_apply_func_t)(ccr_can_apply_bitgroom), /**< Callback to determine if current variable meets filter criteria */
    (H5Z_set_local_func_t)(ccr_set_local_bitgroom), /**< Callback to determine and set per-variable filter parameters */
    (H5Z_func_t)(H5Z_filter_bitgroom),  /**< Function to implement filter */
}};

void ccr_bgr (const int nsd, const int type, const size_t sz, const int has_mss_val, ptr_unn mss_val, ptr_unn op1);
 ...
 * \return size_t
 *
 * \details H5Z_filter_bitgroom() HDF5 BitGroom Filter
 */
    ...
    if(flags & H5Z_FLAG_REVERSE) {
        ...
        switch(datum_size) {
            ...
            default:
                ...
                PUSH_ERR("H5Z_filter_bitgroom", H5E_CALLBACK, errmsg);
                goto error;
                break;
        }
 ...
 * \return htri_t
 *
 * \details ccr_can_apply_bitgroom() Callback to determine if current variable meets filter criteria
 */
htri_t ccr_can_apply_bitgroom(hid_t dcpl, hid_t type, hid_t space) {
    char errmsg[256];
    ...
    if (H5Sis_simple(space) <= 0) {
        ...
        PUSH_ERR("ccr_can_apply_bitgroom", H5E_CALLBACK, errmsg);
        return 0;
    }
 ...
 * \return herr_t
 *
 * \details ccr_set_local_bitgroom() Callback to determine and set per-variable filter parameters
 */
herr_t ccr_set_local_bitgroom(hid_t dcpl, hid_t type, hid_t space) {
    ...
    if (rcd < 0) {
        sprintf(errmsg, "%s filter callback function %s reports H5Pget_filter_by_id() failed to get filter flags and parameters for current variable\n",
                CCR_FLT_NAME, fnc_nm);
        PUSH_ERR("ccr_set_local_bitgroom", H5E_CALLBACK, errmsg);
        return 0;
    }
    ...
    if (data_class < 0) {
        sprintf(errmsg, "%s filter callback function %s reports H5Tget_class() returned invalid data type class identifier = %d for current variable\n",
...
#ifdef CCR_FLT_DBG_INFO
            (void) fprintf(stdout,
                    "INFO: \"%s\" filter callback function %s reports H5Tget_class() returned data type class identifier = %d != H5T_FLOAT = %d. Attempting to remove quantization filter using H5Premove_filter()...",
    ...
    if (datum_size <= 0) {
        sprintf(errmsg, "%s filter callback function %s reports H5Tget_size() returned invalid datum size = %lu B\n",
                CCR_FLT_NAME, fnc_nm, datum_size);
        PUSH_ERR("ccr_set_local_bitgroom", H5E_CALLBACK, errmsg);
        return 0;
    }
    ...
    if (rcd < 0) {
        sprintf(errmsg, "\"%s\" filter callback function %s reports H5Pfill_value_defined() returns error code = %d\n",
    ...
    {
        (void) fprintf(stdout,
                "INFO: \"%s\" filter callback function %s reports H5Pfill_value_defined() status = %d meaning ... ",
    ...
    if (status == H5D_FILL_VALUE_USER_DEFINED && data_class == H5T_FLOAT) {
        ...
        if (rcd < 0) {
            sprintf(errmsg, "\"%s\" filter callback function %s reports H5Pget_fill_value() returns error code = %d\n",
                    CCR_FLT_NAME, fnc_nm, rcd);
            PUSH_ERR("ccr_set_local_bitgroom", H5E_CALLBACK, errmsg);
            return 0;
        }
    ...
    if (rcd < 0) {
        sprintf(errmsg, "\"%s\" filter callback function %s reports H5Pmodify_filter() unable to modify filter parameters\n",
                CCR_FLT_NAME, fnc_nm);
        PUSH_ERR("ccr_set_local_bitgroom", H5E_CALLBACK, errmsg);
        return 0;
    }
...
/**
 * \brief BitGroom buffer of float values
 *
 ...
 * \return void
 *
 * \details ccr_bgr() BitGroom buffer of float values
 */
void ccr_bgr (const int nsd, const int type, const size_t sz, const int has_mss_val,
        ptr_unn mss_val, ptr_unn op1) {
    ...
    switch (type) {
        case NC_FLOAT:
            ...
            if(bit_xpl_nbr_zro > bit_xpl_nbr_sgn - NCO_PPC_BIT_XPL_NBR_MIN);
                PUSH_ERR("ccr_bgr", H5E_CALLBACK, "assert(bit_xpl_nbr_zro <= bit_xpl_nbr_sgn - NCO_PPC_BIT_XPL_NBR_MIN) failed\n");
            u32_ptr = op1.ui32p;
            /* Create mask */
